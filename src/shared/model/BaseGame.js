...
BaseGame.prototype.onStop = function()
{
    ...
    var size = this.getSize(this.getPresentAvatars().count());//this.room.config.getVariable('worldSize'));//
...
{
    var square = this.perPlayerSize * this.perPlayerSize * (1.1 + this.room.config.getVariable('worldSize')),
        size   = Math.sqrt(square + ((players - 1) * square / 5));
    //var finalSize = size * (1 + ((this.avatars.count() * 2) * this.room.config.getVariable('worldSize')))
