...
/**
 * Remove multipls body from islands
 *
 * @param {Body} body
 */
World.prototype.removeMultipleBody = function(body)
{
    if (!this.active) {
        return [];
    }
    ...
    for (i = -5; i < 6; i++) {
	for (j = -5; j < 6; j++) {
	    var tmpBody = new Body(body.x + i, body.y + j, body.radius, body.data);
	    var b = this.getBody(tmpBody);
	    if (b) {
		this.removeBody(b);
		pts[cpt] = [b.x, b.y];
		cpt++;
