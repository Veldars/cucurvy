...
/**
 * On config is ClockGame
 *
 * @param {SocketClient} client
 * @param {Object} data
 * @param {Function} callback
 */
RoomController.prototype.onConfigIsClockGame = function(client, data, callback)
{
    var success = this.isRoomMaster(client) && this.room.config.setIsClockGame(data.isClockGame);
    ...
    if (success) {
        this.socketGroup.addEvent('room:config:isClockGame', {
            isClockGame: this.room.config.isClockGame
        });
    }
};
