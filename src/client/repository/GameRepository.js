...
GameRepository.prototype.attachEvents = function()
{
    ...
    // Custom
    this.client.on('eraser', this.onEraser);
};
...
GameRepository.prototype.detachEvents = function()
{
    ...
    // Custom
    this.client.off('eraser', this.onEraser);
};
 ...
 * @param {Event} e
 */
GameRepository.prototype.onEraser = function(e)
{
    //this.game.clearTrails();
    var pts = e.detail[0].points;
    var radius = e.detail[0].radius;

    //console.log(pts);
    //   console.log('onErase : ' + x + ' ' + y + ' : ' + radius);

    /*for (i = this.game.avatars.items.length - 1; i >= 0; i--) {
	this.game.avatars.items[i].trail.clear();
	this.game.avatars.items[i].canvas.clear();
	}*/
    //    this.game.background.color(this.game.backgroundColor);
    var scale = this.game.background.scale;
    for(var i = 0; i < pts.length; i++) {
	this.game.background.drawCircle(pts[i][0] * scale, pts[i][1] * scale, radius * scale * 1.4, this.game.backgroundColor);
    }
    //this.game.background.drawLineScaled(pts, radius * 2, this.game.backgroundColor ,'round');
    //this.game.background.context.fillRect(x * this.game.background.scale - 15, y * this.game.background.scale - 15, 30, 30);
};
